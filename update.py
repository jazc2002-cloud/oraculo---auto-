import json
print("V88.5 COMPLETO 42 EVENTOS")

def momio(p):
    pr=max(p/100,0.01)
    return round(1/(pr*1.045),2), round(1/pr,2), f"{round((pr*1/(pr*1.045)-1)*100,1)}%"

raw=[
("11/09","MX J7-J8","America","Monterrey","Azteca 19:00 TUDN HOY",54,24,22,"WWLWD","WWWWW",8,3,9,4,"H2H America 2-2-1"),
...
("15/09","MX FEM J9-J10","Monterrey Femenil","Tigres UANL Femenil","BBVA FEM 20:00 FOX",46,26,28,"WWWWL",8,3,"WWWWW",9,2,"Clasico Regio 2-2-1"),
...
games={}
for r in raw:
    fecha,liga,home,away,tv,ph,pd,pa,fh,ghf,ghc,fa,gaf,gac,h2h=r
    xgh=round(0.8+ghf/5,2); xga=round(0.7+gaf/5,2)
    porque=f"ULT5 REAL: {home} {fh} GF:{ghf} GC:{ghc} xG:{xgh} | {away} {fa} GF:{gaf} GC:{gac} xG:{xga} | {h2h} | PROB {ph}/{pd}/{pa}=100%"
    mid=f"{fecha}_{liga}_{home}_{away}"
    ms=[]
    for op,prob in [(f"{home} Gana",ph),("Empate",pd),(f"{away} Gana",pa)]:
        if prob<=0: continue
        m,j,ev=momio(prob)
        ms.append({"op":op,"prob":f"{prob}%","momio":f"@{m}","justo":f"@{j}","ev":ev,"porque":porque,"top":False})
    ms=sorted(ms,key=lambda x: float(x["ev"].replace("%","")),reverse=True)
    for i in range(len(ms)): ms[i]["top"]=i<2
    mejor=ms[0]
    games[mid]={"title":f"{liga} {home} vs {away}","title_short":f"{home} vs {away}","tv":tv,"liga":liga,"liga_hoy":"HOY" if "HOY" in tv else liga,"home":home,"away":away,"fecha":fecha,"prob":ph,"prob_d":pd,"prob_a":pa,"xg_h":xgh,"xg_a":xga,"momio":f"@{mejor['momio'].replace('@','')}","justo":f"@{mejor['justo'].replace('@','')}","ev":mejor["ev"],"mejor":{"pick":f"{mejor['op']} {mejor['momio']} {mejor['prob']}","porque":porque,"momio":mejor["momio"]},"mejores_lista":ms[:2],"mercados":ms,"marcadores":[{"score":"2-1","prob":"14%","momio":"@8.50"}]}

gj=json.dumps(games,ensure_ascii=False)
html="""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V88.5</title>
<style>
...
function counts(){var c={};var c80=0;order.forEach(function(l){c[l]=0;});Object.values(games).forEach(function(g){if(order.indexOf(g.liga)>=0)c[g.liga]++;if(g.liga_hoy==="HOY")c["HOY"]=(c["HOY"]||0)+1;if(g.prob>=60)c80++;});c["80%+"]=c80;return c;}
function renderFiltros(){var c=counts();var h='';h+='<button class="btn-green '+(currentFiltro==='80%+'?'active':'')+'" onclick="setFiltro(\\'80%+\\')">60%+ ('+c['80%+']+')</button>';h+='<button class="btn-blue '+(currentFiltro==='HOY'?'active':'')+'" onclick="setFiltro(\\'HOY\\')">HOY ('+(c['HOY']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='MX J7-J8'?'active':'')+'" onclick="setFiltro(\\'MX J7-J8\\')">MX ('+(c['MX J7-J8']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='MX FEM J9-J10'?'active':'')+'" onclick="setFiltro(\\'MX FEM J9-J10\\')">FEM ('+(c['MX FEM J9-J10']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='EUROPA'?'active':'')+'" onclick="setFiltro(\\'EUROPA\\')">EUROPA ('+(c['EUROPA']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='UCL J1-J2'?'active':'')+'" onclick="setFiltro(\\'UCL J1-J2\\')">UCL ('+(c['UCL J1-J2']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='MLS'?'active':'')+'" onclick="setFiltro(\\'MLS\\')">MLS ('+(c['MLS']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='BEIS FINAL'?'active':'')+'" onclick="setFiltro(\\'BEIS FINAL\\')">BEIS ('+(c['BEIS FINAL']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='NFL S2-S3'?'active':'')+'" onclick="setFiltro(\\'NFL S2-S3\\')">NFL ('+(c['NFL S2-S3']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='F1 BAKU'?'active':'')+'" onclick="setFiltro(\\'F1 BAKU\\')">F1 ('+(c['F1 BAKU']||0)+')</button>';h+='<button class="btn-dark '+(currentFiltro==='BOX/UFC'?'active':'')+'" onclick="setFiltro(\\'BOX/UFC\\')">BOX/UFC ('+(c['BOX/UFC']||0)+')</button>';document.getElementById('filtros').innerHTML=h;}
function setFiltro(f){currentFiltro=f;renderFiltros();renderLista();}
function renderLista(){var list=Object.entries(games);if(currentFiltro==="HOY"){list=list.filter(function(e){return e[1].liga_hoy==="HOY";});}else if(currentFiltro==="80%+"){list=list.filter(function(e){return e[1].prob>=60;});}else{list=list.filter(function(e){return e[1].liga===currentFiltro;});}var h="";list.forEach(function(en){var id=en[0];var g=en[1];h+='<div class="card-outer"><div class="card-top">'+g.title.toUpperCase()+'</div><div class="card-mid"><span>'+g.tv+'</span><span class="badge-ev">'+g.ev+' REAL</span></div><div class="card-bot" onclick="openGame(\\''+id+'\\')">'+g.home.toUpperCase()+' ML '+g.momio+' '+g.prob+'% REAL - TOCA</div></div>';});document.getElementById('lista').innerHTML=h;}
...
function showTab(t){document.querySelectorAll('.tabm button').forEach(function(b){b.classList.remove('active');});document.getElementById('bt_'+t).classList.add('active');var g=window.currentGame;var h="";if(t==="todas"){h=g.mercados.map(function(m){return '<div class="mercado"><div><b>'+m.op+'</b><br><small style="color:#8aa">'+m.porque+'</small><br><small style="color:#ffcc00">% REAL: '+m.prob+' | EV: '+m.ev+'</small></div><div><b style="color:#00ff88">'+m.momio+'</b></div></div>';}).join('');}else{h=g.mejores_lista.map(function(m){return '<div class="mercado"><div><b>'+m.op+' - '+m.prob+' REAL | '+m.ev+'</b><br><small>'+m.porque+'</small></div></div>';}).join('');}document.getElementById('mercados').innerHTML=h;}
renderFiltros();renderLista();
</script></body></html>
"""
open("index.html","w",encoding="utf-8").write(html.replace("__GAMES_JSON__",gj).replace("__TOTAL__",f"{len(games)} EVENTOS"))
print(f"LISTO V88.5 {len(games)}")
