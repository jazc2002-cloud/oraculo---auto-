...
print("V88 COMPLETO 12-21 SEP - EUROPA + F1 + FEM EUROPA")
...
extras = [
    ...
    ("12_6","12/09 - Garcia vs Benn WBC HOY","BOX/UFC","Ryan Garcia","Conor Benn","Vegas Paramount+ 18:00 HOY",81,"BOX/UFC"),
    ("12_7","12/09 - Sultanes vs Diablos J2 Final","BEIS FINAL","Sultanes","Diablos Rojos","LMB J2 19:00 HOY",80,"BEIS FINAL"),
    ("12_8","12/09 - Inter Miami vs Nashville Messi","MLS","Inter Miami","Nashville SC","Apple TV 19:30 HOY",69,"MLS"),
    # F1 BAKU COMPLETO
    ...
    ("eu_12_4","12/09 - Inter vs Juventus","EUROPA","Inter Milan","Juventus","Serie A San Siro 13:00 ESPN HOY",76,"EUROPA"),
    ...
    ("13_3","13/09 - Pachuca Fem vs Toluca Fem","MX FEM J9-J10","Pachuca Femenil","Toluca Femenil","FOX 19:00",74,"MX FEM J9-J10"),
    ...
    ("eu_13_2","13/09 - Atletico Madrid vs Villarreal","EUROPA","Atletico Madrid","Villarreal","LaLiga Metropolitano 13:00 ESPN",72,"EUROPA"),
    ...
    # --- 14/09 DOM NFL W1 + EUROPA ---
    ("14_nfl1","14/09 - Ravens vs Colts NFL W1","NFL S2-S3","Baltimore Ravens","Indianapolis Colts","Indy 13:00 CBS",68,"NFL S2-S3"),
    ("14_nfl2","14/09 - Bills vs Texans W1","NFL S2-S3","Buffalo Bills","Houston Texans","Houston 13:00 CBS",71,"NFL S2-S3"),
    ...
    # --- 15-16-17-18 UCL + EUROPA ---
    ("15_1","15/09 - Broncos vs Chiefs MNF","NFL S2-S3","Denver Broncos","Kansas City Chiefs","MNF 20:15 ESPN",70,"NFL S2-S3"),
    ...
    ("16_3","16/09 - Juventus vs Dortmund UCL","UCL J1-J2","Juventus","Borussia Dortmund","TNT 13:00 UCL",76,"UCL J1-J2"),
    ("16_4","16/09 - Athletic vs Arsenal UCL","UCL J1-J2","Athletic Bilbao","Arsenal","TNT 13:00 UCL",74,"UCL J1-J2"),
    ...
    ("17_3","17/09 - Frankfurt vs Galatasaray UCL","UCL J1-J2","Eintracht Frankfurt","Galatasaray","TNT 13:00 UCL",72,"UCL J1-J2"),
    ("18_1","18/09 - Liverpool vs Atletico UCL","UCL J1-J2","Liverpool","Atletico Madrid","TNT 13:00 UCL",77,"UCL J1-J2"),
    ("18_2","18/09 - Newcastle vs Barcelona UCL FEM","UCL J1-J2","Newcastle","Barcelona","TNT 13:00 UCL",75,"UCL J1-J2"),
    # --- 19-20-21 EUROPA JORNADA SIGUIENTE ---
    ...
    ("eu_21_3","21/09 - Milan vs Udinese","EUROPA","AC Milan","Udinese","Serie A San Siro 13:00 ESPN",76,"EUROPA"),
...
for id_,title,liga,home,away,tv,prob,tag in extras:
    ...
    porque = f"{home} xG {xg} vs {xg2} {away} | Pos {poss}% vs {poss2}% | Forma {forma} vs {forma2} | 12-21 SEP REAL"
    mercados=[
        {"op":f"{home} Gana","prob":f"{prob}%","momio":f"@{m}","justo":f"@{j}","ev":ev,"porque":porque,"top":True},
        {"op":"Empate","prob":f"{22+(prob%10)}%","momio":f"@3.20","justo":f"@2.95","ev":"+5%","porque":f"Empate historico","top":False},
        {"op":f"{away} Gana","prob":f"{100-prob-22}%","momio":f"@{round(3.5+random.random(),2)}","justo":f"@3.2","ev":"-3%","porque":f"{away} xG {xg2}","top":False},
        {"op":"Over 2.5 Goles","prob":"58%","momio":"@1.95","justo":"@1.80","ev":"+8%","porque":f"xG combinado {round(xg+xg2,1)}","top":False},
    ]
...
html=f"""<!DOCTYPE html><html><head><meta charset=UTF-8><meta name=viewport content=width=device-width,initial-scale=1><title>V88 12-21 SEP COMPLETO</title>
<style>
...
<div class="top-banner">✅ V88 12-21 SEP COMPLETO - {len(games)} EVENTOS - EUROPA 5 LIGAS + FEM EUROPA + F1 BAKU + UCL + BOX REAL + MX + NFL W1 - 12 AL 21 SEP 2026</div>
...
var current="HOY";
function counts(){{var c={{}}; c["HOY"]=0; c["80%+"]=0; c["EUROPA"]=0; c["EURO FEM"]=0; c["F1 BAKU"]=0; Object.values(games).forEach(g=>{{if(g.liga_hoy==="HOY") c["HOY"]++; if(g.prob>=80) c["80%+"]++; if(g.liga==="EUROPA") c["EUROPA"]++; if(g.liga==="EURO FEM") c["EURO FEM"]++; if(g.liga==="F1 BAKU") c["F1 BAKU"]++;}}); return c;}}
function renderFiltros(){{var c=counts(); var h=''; h+=`<button class="btn-green ${{current==='80%+'?'active':''}}" onclick="setF('80%+')">🔥 80%+ (${{c['80%+']}})</button>`; h+=`<button class="btn-yellow ${{current==='SUPER'?'active':''}}" onclick="setF('SUPER')">🏆 SUPER</button>`; h+=`<button class="btn-blue ${{current==='HOY'?'active':''}}" onclick="setF('HOY')">🔴 HOY 12/09 (${{c['HOY']}})</button>`; h+=`<button class="btn-dark ${{current==='EUROPA'?'active':''}}" onclick="setF('EUROPA')">🇪🇺 EUROPA 5 LIGAS (${{c['EUROPA']}})</button>`; h+=`<button class="btn-dark ${{current==='EURO FEM'?'active':''}}" onclick="setF('EURO FEM')">👩 EUROPA FEM (${{c['EURO FEM']}})</button>`; h+=`<button class="btn-dark ${{current==='F1 BAKU'?'active':''}}" onclick="setF('F1 BAKU')">🏎️ F1 BAKU (${{c['F1 BAKU']}})</button>`; h+=`<button class="btn-dark ${{current==='UCL J1-J2'?'active':''}}" onclick="setF('UCL J1-J2')">🏆 UCL 12-21</button>`; h+=`<button class="btn-dark ${{current==='MX J7-J8'?'active':''}}" onclick="setF('MX J7-J8')">🇲🇽 MX</button>`; h+=`<button class="btn-dark ${{current==='BOX/UFC'?'active':''}}" onclick="setF('BOX/UFC')">🥊 BOX REAL</button>`; document.getElementById('filtros').innerHTML=h;}}
...
function openG(id){{var g=games[id]; document.getElementById('mtitle').innerText=g.title; document.getElementById('mtv').innerText=g.tv+' | '+g.porque; document.getElementById('modal').style.display='block'; var h=''; g.mercados.forEach(m=>{{h+=`<div class="mercado"><div><b>${{m.op}}</b><br><small>${{m.porque}}</small></div><div><b style="color:#00ff88">${{m.momio}}</b></div></div>`;}}); document.getElementById('mercados').innerHTML=h;}}
renderFiltros(); renderLista();
</script></body></html>"""

with open("index.html","w",encoding="utf-8") as f:
    f.write(html)
print(f"LISTO V88 12-21 SEP COMPLETO {len(games)} EVENTOS")
