import json
print("V88.3 TODAS COMPETENCIAS + TODOS EVENTOS + ULT5 H2H COND")

def power(n):
    b={"Tigres UANL Femenil":92,"Monterrey Femenil":90,"America Femenil":88,"Pachuca Femenil":86,"Chivas Femenil":84,"America":85,"Monterrey":84,"Toluca":82,"Cruz Azul":81,"Tigres UANL":83,"Real Madrid":90,"Barcelona":89,"Bayern":88,"Man City":87,"Liverpool":86,"PSG":86,"Sultanes":80,"Diablos":85,"Cowboys":82,"Chiefs":88,"Verstappen":95,"Canelo":94}
    for k,v in b.items():
...
def momio(p):
    pr=p/100
    if pr<=0: pr=0.01
    j=round(1/pr,2); m=round(1/(pr*1.045),2); ev=(pr*m-1)*100
...
("12/09","MX J7-J8","Tigres UANL","Chivas","Universitario 19:00 TUDN",52,25,23,"WWLWD",8,3,"LWWWD",5,7,"H2H Tigres 3-1-1 Volcan 42k"),
...
("13/09","MX FEM J9-J10","Atlas Femenil","Leon Femenil","Jalisco FEM 17:00 VIX",42,28,30,"DLWWL",4,6,"WDLWL",5,7,"H2H 2-2-1 Leon visita fuerte"),
...
("13/09","EUROPA","Man City","Man United","Etihad 08:30 ESPN",55,24,21,"WWWWL",10,3,"WWLWL",7,6,"H2H City 3-2 derby"),
...
("14/09","NFL S2-S3","Kansas City Chiefs","Baltimore Ravens","Arrowhead NFL 19:00 ESPN",54,0,46,"WWWWL",135,89,"WWLWW",122,95,"H2H ult5: Chiefs 3-2 Mahomes vs Lamar"),
...
for g in all_games:
    ...
    games[g["id"]]={"title":f"{g['liga_hoy']} {g['title']} - {g['liga'].split()[0]}","title_short":g["title"],"tv":g["tv"],"liga":g["liga"],"liga_hoy":g["liga_hoy"],"home":g["home"],"away":g["away"],"fecha":g["fecha"],"prob":g["prob_h"],"prob_d":g["prob_d"],"prob_a":g["prob_a"],"xg_h":g["xg_h"],"xg_a":g["xg_a"],"momio":mejor["momio"],"justo":mejor["justo"],"ev":mejor["ev"],"valor":mejor["valor"],"mejor":{"pick":f"{mejor['op']} {mejor['momio']} {mejor['prob']} REAL","porque":mejor["porque"],"momio":mejor["momio"],"justo":mejor["justo"],"valor":mejor["valor"]},"mejores_lista":mejores,"mercados":mercados,"marcadores":[{"score":"2-1","prob":"14%","momio":"@8.50","top":True}],"parlays":[{"picks":f"{mercados[0]['op']}","momio":"@2.85","prob":f"{g['prob_h']-12}%","efec":f"{g['prob_h']}%","detalle":g["porque"]}]}

games_json=json.dumps(games,ensure_ascii=False)
tpl="""<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>V88.3</title>
<style>body{background:#050a0a;color:#fff;font-family:Arial;margin:0;padding:6px}.top-banner{background:#0a2a1a;border:2px dashed #00ff88;color:#00ff88;padding:12px;border-radius:14px;text-align:center;font-weight:800;font-size:11px;margin-bottom:10px}.filtros{background:#0a1414;border:1px solid #1a2a2a;border-radius:16px;padding:10px;display:flex;flex-wrap:wrap;gap:6px;justify-content:center;margin-bottom:12px}.filtros button{border:none;padding:8px 13px;border-radius:18px;font-weight:800;font-size:11px;cursor:pointer;border:1px solid #222}.btn-green{background:#00e676;color:#000}.btn-blue{background:#0f2a4a;color:#4fc3f7;border:1px solid #1a4a7a}.btn-dark{background:#1b2a2a;color:#b0c4c4}.filtros button.active{outline:2px solid #00ff88;box-shadow:0 0 10px #00ff88;transform:scale(1.05)}.card-outer{background:#071a14;border:2px solid #00ff88;border-radius:16px;padding:5px;margin:10px 2px}.card-top{background:#0e2233;border-radius:10px;padding:8px 10px;margin-bottom:4px;font-weight:800;color:#4fc3f7;font-size:11px}.card-mid{background:#1a1a0a;border-radius:8px;padding:6px 10px;margin-bottom:4px;color:#ffcc66;font-size:10px;display:flex;justify-content:space-between}.card-bot{background:linear-gradient(90deg,#0a4a2a,#0f7a3a);border:1px solid #00ff88;border-radius:10px;padding:10px;text-align:center;color:#aaffcc;font-weight:900;font-size:11px;cursor:pointer}.modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:99;padding:10px;overflow:auto}.modal-content{background:#0a1818;border:2px solid #00ff88;border-radius:16px;padding:14px;max-width:620px;margin:10px auto}.tabm{display:flex;gap:5px;overflow:auto;margin:12px 0}.tabm button{background:#162a2a;color:#8aa;border:1px solid #234;padding:7px 12px;border-radius:14px;white-space:nowrap;font-size:11px}.tabm button.active{background:#00ff88;color:#000;font-weight:800}.mercado{background:#0e2a2a;border:1px solid #1a4a4a;border-radius:10px;padding:10px;margin:7px 0;font-size:12px;display:flex;justify-content:space-between;align-items:center}.badge-ev{background:#00ff88;color:#000;padding:2px 6px;border-radius:8px;font-weight:800;font-size:10px}</style></head><body>
<div class="top-banner">✅ V88.3 TODAS LAS COMPETENCIAS - __TOTAL__ EVENTOS - FORMATO ORIGINAL + ULT5 H2H + % EFECTIVO</div>
<div class="filtros" id="filtros"></div><div id="lista"></div>
<div class="modal" id="modal"><div class="modal-content"><button onclick="document.getElementById('modal').style.display='none'" style="float:right;background:#222;color:#fff;border:1px solid #444;padding:6px 10px;border-radius:8px">X</button><h2 id="mtitle" style="color:#4fc3f7;font-size:14px;margin:0"></h2><div id="mtv" style="color:#ffcc33;margin:6px 0;font-size:10px"></div><div class="tabm"><button onclick="showTab('todas')" id="bt_todas" class="active">TODAS</button><button onclick="showTab('mejor')" id="bt_mejor">MEJOR + PORQUE</button></div><div id="mercados"></div></div></div>
<script id="games-data" type="application/json">__GAMES_JSON__</script>
<script>
var games=JSON.parse(document.getElementById('games-data').textContent);var order=["HOY","MX J7-J8","MX FEM J9-J10","EUROPA","NFL S2-S3","UCL J1-J2","UEL J1-J2","MLS","BEIS FINAL","F1 BAKU","BOX/UFC"];var currentFiltro="HOY";
...
function renderLista(){var list=Object.entries(games);if(currentFiltro==="HOY"){list=list.filter(e=>e[1].liga_hoy==="HOY");}else if(currentFiltro==="80%+"){list=list.filter(e=>e[1].prob>=60);}else if(order.includes(currentFiltro)){list=list.filter(e=>e[1].liga===currentFiltro);}var html="";list.forEach(entry=>{var id=entry[0];var g=entry[1];html+=`<div class="card-outer"><div class="card-top"> ${g.title.toUpperCase()}</div><div class="card-mid"><span>📺 ${g.tv}</span><span class="badge-ev">${g.ev} REAL</span></div><div class="card-bot" onclick="openGame('${id}')">${g.home.toUpperCase()} ML ${g.momio} ${g.prob}% REAL - TOCA PARA VER TODAS LAS OPCIONES</div></div>`;});document.getElementById('lista').innerHTML=html;}
function openGame(id){var g=games[id];document.getElementById('mtitle').innerText=g.title;document.getElementById('mtv').innerText=g.tv + ' | EV ' + g.ev + ' | ' + g.prob + '/' + g.prob_d + '/' + g.prob_a + '=100%';document.getElementById('modal').style.display='block';window.currentGame=g;showTab('todas');}
function showTab(t){document.querySelectorAll('.tabm button').forEach(b=>b.classList.remove('active'));document.getElementById('bt_'+t).classList.add('active');var g=window.currentGame;var h="";if(t==="todas"){h=g.mercados.map(m=>`<div class="mercado"><div><b>${m.op}</b><br><small style="color:#8aa">${m.porque}</small><br><small style="color:#ffcc00">% REAL: ${m.prob} | EFECTIVO: ${m.efec} | EV: ${m.ev}</small></div><div><b style="color:#00ff88">${m.momio}</b></div></div>`).join('');}else{h=g.mejores_lista.map(m=>`<div class="mercado"><div><b>${m.op} - ${m.prob} REAL | ${m.efec} | ${m.ev}</b><br><small>${m.porque}</small></div></div>`).join('');}document.getElementById('mercados').innerHTML=h;}
renderFiltros();renderLista();
</script></body></html>
"""
final=tpl.replace("__GAMES_JSON__",games_json).replace("__TOTAL__",str(len(games)))
open("index.html","w",encoding="utf-8").write(final)
print(f"LISTO V88.3 - {len(games)} EVENTOS - VERDE")
